...
public Users addUser(Users user) {
    ...
    if (!password.matches(regex)) {
        throw new RuntimeException(
            "Password must contain uppercase, lowercase, number, special symbol and minimum 8 characters"
        );
    }
...
public boolean emailExists(String email) {
	...
	public Optional<Users> getUserByEmail(String email) {
	    return usersRepo.findByEmail(email); // Optional<Users>
	}
	...
	public void clearCart() {
		indoarPlantsrepo1.resetAllQty();
		outdoarPlantsRepo.resetAllQty();
		fruitsVegitablesRepo.resetAllQty();
	    seedsRepo.resetAllQty();
	    soilPesticideRepo.resetAllQty();
	    floweringPlantsRepo.resetAllQty();
	    herbsRepo.resetAllQty();
	    bulbsRepo.resetAllQty();
	    succulentsCactusRepo.resetAllQty();
	    plantAccessoriesRepo.resetAllQty();
	}
